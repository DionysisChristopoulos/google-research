...
def create_gen_dataset_from_images(image_dir, mask_dir, train):
  ...
  for r, m in zip(sorted(glob.glob(image_dir + "/**")), sorted(glob.glob(mask_dir + "/**"))):
    ...
    if train:
      gen.save('D:\\Datasets\\Timeseries_cropped_512\\gen_for_eval\\train\\' + os.path.basename(r) + '.png')
    else:
      gen.save('D:\\Datasets\\Timeseries_cropped_512\\gen_for_eval\\test\\' + os.path.basename(r) + '.png')
